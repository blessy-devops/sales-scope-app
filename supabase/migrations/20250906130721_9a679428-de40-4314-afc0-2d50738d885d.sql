...
-- Atualiza a função para normalizar os campos de UTM no JOIN (TRIM + LOWER)
...
BEGIN
  ...
  JOIN
    public.attendants AS att ON LOWER(TRIM(sog.utm_medium)) = LOWER(TRIM(att.utm_identifier))
  WHERE
